...
function TextContainer({ defaultText = "My Guy", defaultSize = "xs", defaultTextLayout = "horizontal", alignText = "center" }: Props) {
    ...
    const [alignChoice, setAlignChoice] = React.useState('center');
    ...
    return (
        <div ref={ref} onClick={turnOnEditMode} onMouseOver={mouseInContainer} onMouseLeave={mouseLeftContainer} className={`text-container ${size} ${editMode ? "edit-on" : ""} ${textLayout === "vertical" ? "vertical" : ""} ${alignText === "left" ? "text-left" : ""}`}>
            ...
            <BasicTools />
            {editMode && <Tooltip alignChoice={alignChoice} setAlignChoice={setAlignChoice} parentRef={ref} fontColor={fontColor} setFontColor={setFontColor} turnOffEditMode={turnOffEditMode} parentContainer={ref} defaultFont={fontFamily} setCurrentFont={setFontFamily} />}
...
function EditForm({ text, size, saveNewText, defaultText, alignText = 'center' }) {
    ...
    return (
        <form onSubmit={SubmitForm}>
            <input onKeyDown={(e) => { e.stopPropagation(); if (e.key === "Escape") { saveNewText(defaultText) } }} ref={ref} className={size} onChange={updateText} type="text" value={draftText}></input>
        </form>
    )
}
